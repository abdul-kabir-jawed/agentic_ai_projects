...
- Sub-agents location: `.gemini/sub_agents/`
...
evolution-of-todo/
├── .gemini/
│   ├── commands/              # Gemini-specific commands (TOML)
│   ├── skills/                # Reusable workflow patterns
│   └── sub_agents/            # Autonomous decision agents
├── .specify/
...
|-------|---------|----------|
| API Documenter | Generate OpenAPI/GraphQL documentation | `.gemini/skills/api-documenter.md` |
| Docker Image Builder | Create optimized Dockerfiles | `.gemini/skills/docker-image-builder.md` |
| Helm Chart Creator | Generate Kubernetes Helm charts | `.gemini/skills/helm-chart-creator.md` |
| Test Writer | Generate comprehensive test suites | `.gemini/skills/test-writer.md` |

## Available Sub-Agents

| Sub-Agent | Purpose | Location |
|-----------|---------|----------|
| Architecture Compliance Auditor | Validate architecture decisions | `.gemini/sub_agents/architecture-compliance-auditor.md` |
| Security Validator | Check security requirements | `.gemini/sub_agents/security-validator.md` |
...
6. **Validate after changes** - Check for errors, run validation steps
7. **Invoke skills when applicable** - Use existing skills for repeatable workflows
...
1. **Be precise** - Use exact file paths, specific references
...
6. **Leverage skills** - Use existing skills instead of recreating workflows
...
1. Identify which command/skill is needed for the task
2. Load constitution from `.specify/memory/constitution.md`
3. Check for existing feature context in `specs/` directory
4. Identify which phase of the workflow you're in
5. Execute the appropriate command or invoke relevant skill
