...
class TaskAPI {
  ...
  async sendChatMessage(messages: { role: string; content: string }[]): Promise<string> {
    const { data } = await this.client.post<{ content: string }>('/chat', { messages });
    return data.content;
  }
  ...
  async getApiKeyStatus(): Promise<{
    ...
    const { data } = await this.client.get('/auth/me/api-keys');
    return data;
  }
  ...
  async updateApiKeys(keys: {
    ...
    const { data } = await this.client.put('/auth/me/api-keys', keys);
    return data;
  }

  async deleteApiKeys(): Promise<{ message: string; deleted: boolean }> {
    const { data } = await this.client.delete('/auth/me/api-keys');
    return data;
  }
  ...
  async checkChatApiKeyAccess(): Promise<{
    ...
    const { data } = await this.client.get('/chat/api-key-check');
    return data;
  }
}
